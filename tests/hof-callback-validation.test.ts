...
describe('higher-order callback validation', () => {
  const withInc = () =>
    bonsai()
      .use(arrays)
      .addFunction('inc', (x: unknown) => (x as number) + 1)
  ...
  describe('stdlib transforms throw on a non-function callback', () => {
    it('map throws a typed error instead of silently returning the input', () => {
      const expr = withInc()
      expect(() =>
        expr.evaluateSync('items |> map(inc(.x))', { items: [{ x: 1 }, { x: 2 }] }),
      ).toThrow(BonsaiTypeError)
      expect(() => expr.evaluateSync('items |> map(inc(.x))', { items: [{ x: 1 }] })).toThrow(
        /lambda/iu,
      )
    })
    ...
    it('filter/find/some/every throw on a non-function callback', () => {
      ...
      for (const t of ['filter', 'find', 'some', 'every']) {
        expect(() => expr.evaluateSync(`items |> ${t}(inc(.x))`, { items: [{ x: 1 }] }), t).toThrow(
          BonsaiTypeError,
        )
      }
    })
  ...
  describe('no-argument transform forms still work', () => {
    ...
    it('filter with no arg keeps truthy values', () => {
      expect(expr.evaluateSync('items |> filter', { items: [0, 1, '', 'a', null, 2] })).toEqual([
        1,
        'a',
        2,
      ])
  ...
  describe('valid lambda callbacks are unaffected', () => {
    ...
    it('a function that legitimately returns a callback still works', () => {
      ...
      const expr2 = bonsai()
        .use(arrays)
        .addFunction('identity', (f: unknown) => f)
      expect(
        expr2.evaluateSync('items |> map(identity(.x))', { items: [{ x: 1 }, { x: 2 }] }),
      ).toEqual([1, 2])
    })
  })
  ...
  describe('method-path HOFs throw a typed error (not a raw TypeError)', () => {
    ...
    it('every higher-order method rejects a non-function callback', () => {
      ...
      for (const m of ['filter', 'find', 'some', 'every', 'findIndex', 'flatMap']) {
        expect(() => expr.evaluateSync(`items.${m}(inc(.x))`, { items: [{ x: 1 }] }), m).toThrow(
          BonsaiTypeError,
        )
      }
    })
  ...
  describe('async parity', () => {
    it('async HOFs throw on a non-function callback too', async () => {
      const expr = withInc()
      await expect(expr.evaluate('items |> map(inc(.x))', { items: [{ x: 1 }] })).rejects.toThrow(
        BonsaiTypeError,
      )
      await expect(expr.evaluate('items.filter(inc(.x))', { items: [{ x: 1 }] })).rejects.toThrow(
        BonsaiTypeError,
      )
    })
  })
